...
public class GUI {
	...
	protected void createContents() {
		...
		shlStar.addShellListener(new ShellAdapter() {
			...
			public void shellClosed(ShellEvent e) {
				...
				messageBox.setMessage("确定要退出系统吗?");
		...
		lblNewLabel_3.setFont(SWTResourceManager.getFont("华文行楷", 14, SWT.BOLD));
		...
		lblNewLabel_2.setForeground(SWTResourceManager
				.getColor(SWT.COLOR_DARK_MAGENTA));
		lblNewLabel_2.setFont(SWTResourceManager.getFont("华文行楷", 14, SWT.BOLD));
		...
		lblNewLabel_4.setForeground(SWTResourceManager
				.getColor(SWT.COLOR_DARK_MAGENTA));
		lblNewLabel_4.setFont(SWTResourceManager.getFont("华文行楷", 14, SWT.BOLD));
		...
		lblNewLabel_5.setForeground(SWTResourceManager
				.getColor(SWT.COLOR_DARK_MAGENTA));
		lblNewLabel_5.setFont(SWTResourceManager.getFont("华文行楷", 14, SWT.BOLD));
		...
		lblNewLabel_6.setForeground(SWTResourceManager
				.getColor(SWT.COLOR_DARK_MAGENTA));
		lblNewLabel_6.setFont(SWTResourceManager.getFont("华文行楷", 14, SWT.BOLD));
		...
		for (int i = 0; i < tablegroups.size(); i++) {
			...
			for (int j = 0; j < tables.size(); j++) {
				...
				DragSource source = new DragSource(button_n, DND.DROP_MOVE
						| DND.DROP_COPY);
				source.setTransfer(new Transfer[] { TextTransfer.getInstance() });// 设置传输载体为文本型
						...
						if (stateString.equals("EMPTY")) {
							...
							double minutes = (date.getMinutes() - walkinBill
									...
									+ (date.getHours() - walkinBill.getDate()
											.getHours()) * 60;
							lblNewLabel_4.setText(String.valueOf(minutes) + "分");
							lblNewLabel_5.setText(String.valueOf(wholemoney)
									+ "元");
							lblNewLabel_6.setText("开单");
						} else {
							List<Integer> bookbillids = uml.middleclass.Table
							...
							for (k = 0; k < bookbillids.size(); k++) {
								if (Book.getBook(bookbillids.get(k)).getState()
									...
									lblNewLabel_5.setText(String
											.valueOf(wholemoney) + "元");
									lblNewLabel_6.setText("预订到达");
									break;
								}
							}
		...
		toolBar.setFont(SWTResourceManager.getFont("微软雅黑", 9, SWT.BOLD));
				...
				if (ptableid == 0) {
					...
					if (uml.middleclass.Table.search(ptableid).getState()
						...
						if (walkinbillshowsList.size() == 0) {
							...
							int bookbillid = 0;
							shlStar.setEnabled(false);
							点菜 window = new 点菜(ptableid, walkinbillid,
						...
						for (k = 0; k < bookbillids.size(); k++) {
							if (Book.getBook(bookbillids.get(k)).getState()
								...
								if (resultsStrings.size() == 0) {
									...
									shlStar.setEnabled(false);
									结账 window = new 结账(ptableid, walkinbillid,
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (ptableid == 0) {
					...
					if (uml.middleclass.Table.search(ptableid).getState()
						...
						if (walkinbillshowsList.size() == 0) {
							shlStar.setEnabled(false);
							点菜 window = new 点菜(ptableid, walkinbillid, 0);
							window.open();
							shlStar.setEnabled(true);
							display(tabFolder);
						} else {
						...
						for (k = 0; k < bookbillids.size(); k++) {
							if (Book.getBook(bookbillids.get(k)).getState()
								...
								if (resultsStrings.size() == 0) {
									shlStar.setEnabled(false);
									点菜 window = new 点菜(ptableid, 0, bookbillids
											.get(k));
									window.open();
									shlStar.setEnabled(true);
									display(tabFolder);
		...
		ymd.setFont(SWTResourceManager.getFont("华文新魏", 14, SWT.BOLD));
		...
		hms.setFont(SWTResourceManager.getFont("华文新魏", 14, SWT.BOLD));
			...
			public void run() {
				while (true) {
					...
					display.asyncExec(new Runnable() {
						@Override
						public void run() {
							Date date = new Date(System.currentTimeMillis()); // 定义显示时间的格式
		...
		label_1.setFont(SWTResourceManager.getFont("华文行楷", 14, SWT.BOLD));
		label_1.setBounds(10, 37, 92, 23);
		label_1.setText("\u6700\u4F4E\u6D88\u8D39\uFF1A");

		Label label_2 = new Label(composite, SWT.NONE);
		label_2.setFont(SWTResourceManager.getFont("华文行楷", 14, SWT.BOLD));
		label_2.setBounds(10, 89, 92, 27);
		label_2.setText("\u8FDB\u5E97\u65F6\u95F4\uFF1A");

		Label label_3 = new Label(composite, SWT.NONE);
		label_3.setFont(SWTResourceManager.getFont("华文行楷", 14, SWT.BOLD));
		label_3.setBounds(10, 141, 92, 17);
		label_3.setText("\u5DF2\u7528\u65F6\u95F4\uFF1A");

		Label label_4 = new Label(composite, SWT.NONE);
		label_4.setFont(SWTResourceManager.getFont("华文行楷", 14, SWT.BOLD));
		label_4.setBounds(10, 193, 92, 17);
		label_4.setText("\u6D88\u8D39\u91D1\u989D\uFF1A");

		Label label_5 = new Label(composite, SWT.NONE);
		label_5.setFont(SWTResourceManager.getFont("华文行楷", 14, SWT.BOLD));
	...
	private class MyDragSourceListener implements DragSourceListener {
		...
		public void dragStart(DragSourceEvent event) {
			button100 = getSource(event);// getSource是自定义方法
		...
		public void dragSetData(DragSourceEvent event) {
			if (TextTransfer.getInstance().isSupportedType(event.dataType)) {// 是否支持拖拽的数据类型
	...
	private class MyDropTargetListener implements DropTargetListener {
		...
		public void drop(DropTargetEvent event) {
			if (TextTransfer.getInstance().isSupportedType(
				...
				Button text = (Button) target.getControl();// 即targetText
				// text.setText(str);
