...
public class µã²Ë {
	...
	public µã²Ë(int ptableid, int walkinbillid, int bookbillid) {
		setPtableid(ptableid);
		setWalkinbillid(walkinbillid);
		setBookbillid(bookbillid);
	}

	public µã²Ë(int ptableid, int walkinbillid, int bookbillid, float money,
	...
	protected void createContents() {
		...
		shell.setImage(SWTResourceManager
				.getImage(µã²Ë.class, "/pic/zhuozi4.png"));
		shell.setBackgroundMode(SWT.INHERIT_FORCE);
		shell.setBackgroundImage(SWTResourceManager.getImage(µã²Ë.class,
				"/pic/\u80CC\u666F0.jpg"));
		...
		table.setBackgroundImage(SWTResourceManager.getImage(µã²Ë.class,
				"/pic/\u80CC\u666F1.jpg"));
		...
		tree.setBackgroundImage(SWTResourceManager.getImage(µã²Ë.class,
				"/pic/\u80CC\u666F1.jpg"));
		...
		table_1.setBackgroundImage(SWTResourceManager.getImage(µã²Ë.class,
				"/pic/\u80CC\u666F1.jpg"));
		...
		btnNewButton.setImage(SWTResourceManager.getImage(µã²Ë.class,
				"/pic/\u7BAD\u59342.png"));
		...
		label.setFont(SWTResourceManager.getFont("Î¢ÈíÑÅºÚ", 15, SWT.BOLD));
		...
		button_1.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				if(getBookbillid() == 0){
					uml.middleclass.WalkinBill.checkout(getWalkinbillid(), getPtableid());
				}else{
		...
		button_1.setFont(SWTResourceManager.getFont("Î¢ÈíÑÅºÚ", 12, SWT.BOLD));
		button_1.setBounds(352, 430, 80, 27);
		button_1.setText("\u7ED3\u8D26");
		...
		button_2.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				if(getWalkinbillshowsList() == null){
					...
					for (int i = 0; i < getWalkinbillshowsList().size(); i += 3) {
						uml.middleclass.Table.deleteDishs(getPtableid(),
								getWalkinbillshowsList().get(i), getBookbillid(),
								getWalkinbillid());
					}
				}
				int count = table_1.getItemCount();
				for (int i = 0; i < count; i++) {
					uml.middleclass.Table.addDishs(getPtableid(), table_1
							.getItem(i).getText(0), Integer.parseInt(table_1
							.getItem(i).getText(2)), "description",
							getBookbillid(), getWalkinbillid());
				}
				shell.close();
			}
		});
		button_2.setFont(SWTResourceManager.getFont("Î¢ÈíÑÅºÚ", 12, SWT.BOLD));
		button_2.setBounds(454, 430, 80, 27);
		button_2.setText("\u5173\u95ED\u4FDD\u5B58");
		...
		button_3.setFont(SWTResourceManager.getFont("Î¢ÈíÑÅºÚ", 12, SWT.BOLD));
		...
		button_4.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				lblNewLabel.setText(String.valueOf(displaymoney) + "Ôª");
			}
		});
		...
		if(getWalkinbillshowsList() == null){
			...
			for (int i = 0; i < getWalkinbillshowsList().size(); i = i + 3) {
		...
		lblNewLabel.setText(String.valueOf(getMoney()) + "Ôª");
	}
}
